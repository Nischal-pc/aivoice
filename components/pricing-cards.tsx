    ...
    <div className="space-y-8">
      <div className="flex justify-center">
        <div className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-full">
          <Label
            ...
            className={`px-4 py-2 rounded-full text-sm ${!annual ? "bg-white dark:bg-slate-700 shadow-sm" : ""}`}
          >
            Monthly
          </Label>
          ...
          <Label
            htmlFor="billing-toggle"
            className={`px-4 py-2 rounded-full text-sm flex items-center gap-1 ${annual ? "bg-white dark:bg-slate-700 shadow-sm" : ""}`}
          >
            Annual <span className="text-xs text-green-600 dark:text-green-400 font-medium">Save 20%</span>
          </Label>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((plan, index) => (
          ...
          >
            <Card
              className={`relative h-full flex flex-col ${plan.popular ? "border-2 border-primary shadow-lg shadow-primary/10" : plan.color}`}
            >
              {plan.popular && (
              ...
              <CardFooter>
                <Button
                  asChild
                  className={`w-full ${plan.popular ? "bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-700 hover:to-cyan-700 shadow-lg" : ""}`}
